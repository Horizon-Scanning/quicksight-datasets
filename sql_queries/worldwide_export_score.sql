SELECT
    *,
    CASE
        WHEN availability_adjusted_score_normalized IS NULL THEN NULL
        WHEN availability_adjusted_score_normalized <= -40 THEN 'Red Node'
        WHEN availability_adjusted_score_normalized <= -10 THEN 'Red'
        WHEN availability_adjusted_score_normalized < 0   THEN 'Yellow'
        ELSE                                                    'Green'
    END AS status_color
FROM (
    ...
    base_data AS (
        ...
        SELECT
            ce.*,
            'Brent' AS mapped_commodity_name,
            CASE
                WHEN ce.refmonth IN (1,2,3) THEN 1
                WHEN ce.refmonth IN (4,5,6) THEN 2
                WHEN ce.refmonth IN (7,8,9) THEN 3
        ...
        FROM shippingbi.comtrade_world_exports ce
        INNER JOIN hs_codes_mapping hs ON CAST(ce.cmdcode AS VARCHAR) = hs.hs_code
        WHERE ce.netwgt IS NOT NULL AND hs.mapped_commodity_name = 'Crude Oil'
    ),
