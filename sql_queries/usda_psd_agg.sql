-- Step 0: Define all commodity names that should appear in results
...
-- Step 1.B: Prepare Trading Economics steel and cement data
...
te_unpivoted AS (
    -- For steel and cement, production = supply (no separate supply data)
    -- Aggregate to handle any duplicates before joining in GUI
    SELECT 
        commodity_description, 
        country_name, 
        report_date, 
...
CombinedDataExpanded AS (
    ...
    SELECT 
        ...
        attribute_description,
        value,
        CAST(NULL AS VARCHAR) AS oil_variant
    FROM CombinedDataRaw
    WHERE commodity_description <> 'Crude Oil'
    
    UNION ALL
    
    -- Crude Oil duplicated as Brent
    SELECT 
        commodity_description,
        country_name,
        report_date,
        attribute_description,
        value,
        'Brent' AS oil_variant
    FROM CombinedDataRaw
    WHERE commodity_description = 'Crude Oil'
    
    UNION ALL
    
    -- Crude Oil duplicated as WTI
    SELECT 
        commodity_description,
        country_name,
        report_date,
...
CombinedData AS (
    SELECT 
        ...
        CASE
            ...
            -- Energy commodities (JODI) - Crude Oil split by oil_variant
            ...
            -- Industrial/Chemical commodities (TE)
...
commodities_with_data AS (
    SELECT DISTINCT mapped_commodity_name
    FROM DataWithAvg
),
...
-- Add one NULL row for each commodity with no data
SELECT
    mapped_commodity_name,
