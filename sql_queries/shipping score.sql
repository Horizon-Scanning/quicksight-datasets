SELECT
    *,
    CASE
        WHEN percent_change < -20 THEN 'Red Node'
        WHEN percent_change < -8  THEN 'Red'
        WHEN percent_change <= 0  THEN 'Yellow'
        ELSE                           'Green'
    END AS status_color
FROM (
    ...
    final_scores AS (
        ...
        FROM commodity_weights cw
        JOIN pivoted_momentum pm ON 1=1
    )
    SELECT
        commodity_name,
